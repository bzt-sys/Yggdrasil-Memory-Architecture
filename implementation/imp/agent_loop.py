...
def print_banner() -> None:
    ...
    print(
        ...
        ":trace N, :stats, :diag [N], :graph, :collate [max_items], :export-evidence [dir], :quit"
    )
...
def main() -> None:
    ...
    while True:
        ...
        if user_in.startswith(":collate"):
            parts = user_in.split()
            try:
                max_items = int(parts[1]) if len(parts) > 1 else 64
            except Exception:
                max_items = 64
            result = eng.run_collation_period(max_items=max_items, reason="cli")
            print(f"[COLLATION] reviewed={result.get('reviewed_candidates', 0)} protected={result.get('protected_candidates', 0)} demoted={result.get('demoted_candidates', 0)} proposed={result.get('proposals_created', 0)} promotions={result.get('promotion_evaluations', 0)}")
            _print_hud(eng.hud())
            continue


        if user_in.startswith(":diag"):
            parts = user_in.split()
            try:
                recent = int(parts[1]) if len(parts) > 1 else 5
            except Exception:
                recent = 5
            _print_diag(eng.diagnostics_summary(recent=recent))
            continue
...
def _print_diag(diag: dict) -> None:
    """Print an expanded lifecycle diagnostic snapshot."""
    print("==== YGGDRASIL DIAGNOSTICS ====")
    _print_hud(diag.get("hud", {}))

    cur = diag.get("current_episode")
    if cur:
        print("\n[CURRENT EPISODE]")
        print(
            f"id={str(cur.get('id'))[:8]} status={cur.get('status')} "
            f"open_reason={cur.get('open_reason')} events={cur.get('event_count')} "
            f"features={cur.get('feature_count')}"
        )

    eps = diag.get("recent_episodes") or []
    if eps:
        print("\n[RECENT EPISODES]")
        for ep in eps:
            print(
                f"- {str(ep.get('id'))[:8]} status={ep.get('status')} "
                f"opened={ep.get('opened_step')} closed={ep.get('closed_step')} "
                f"reason={ep.get('close_reason') or ep.get('open_reason')} events={ep.get('event_count')}"
            )

    outs = diag.get("recent_outcomes") or []
    if outs:
        print("\n[RECENT PROVISIONAL OUTCOMES]")
        for o in outs:
            summary = str(o.get("summary", ""))
            if len(summary) > 120:
                summary = summary[:117] + "..."
            print(
                f"- {str(o.get('id'))[:8]} label={o.get('label')} verified={o.get('verified')} "
                f"step={o.get('created_step')} :: {summary}"
            )

    cand_summary = diag.get("candidate_summary") or {}
    if cand_summary:
        print("\n[CANDIDATE SUMMARY]")
        for name, vals in cand_summary.items():
            if vals:
                bits = ", ".join(f"{k}={v}" for k, v in sorted(vals.items()))
                print(f"- {name}: {bits}")

    cands = diag.get("recent_candidates") or []
    if cands:
        print("\n[RECENT CANDIDATES]")
        for c in cands:
            print(
                f"- {str(c.get('id'))[:8]} status={c.get('status')} tier={c.get('memory_tier')} "
                f"preserve={c.get('preservation_state')} organize={c.get('organization_state')} "
                f"authority={c.get('authority_state')} action={c.get('integration_action')} "
                f"rec={c.get('recurrence_count')} var={c.get('variant_count')} "
                f"loc={c.get('locality')} perm={float(c.get('permanence_score') or 0.0):.2f} "
                f"rarity={float(c.get('rarity_score') or 0.0):.2f} sal={float(c.get('salience') or 0.0):.2f}"
            )
            preview = c.get("text_preview")
            if preview:
                print(f"  text: {preview}")

    props = diag.get("recent_governance_proposals") or []
    if props:
        print("\n[RECENT GOVERNANCE PROPOSALS]")
        for p in props:
            print(
                f"- {str(p.get('id'))[:8]} status={p.get('status')} activation={p.get('activation_state')} "
                f"promotion={p.get('promotion_status')} family={p.get('family')} "
                f"loc={p.get('locality')} evidence={p.get('evidence_candidate_count')} "
                f"rec={p.get('recurrence_count')} conf={float(p.get('confidence') or 0.0):.2f}"
            )
            preview = p.get("proposal_preview")
            if preview:
                print(f"  proposal: {preview}")

    locs = diag.get("recent_locality_regions") or []
    if locs:
        print("\n[RECENT LOCALITY REGIONS]")
        for l in locs:
            print(
                f"- {l.get('id')} family={l.get('family')} subfamily={l.get('subfamily')} "
                f"domain={l.get('domain')} status={l.get('status')} depth={l.get('depth')}"
            )

    cols = diag.get("recent_collation_runs") or []
    if cols:
        print("\n[RECENT COLLATION RUNS]")
        for c in cols:
            print(
                f"- {str(c.get('id'))[:8]} step={c.get('step')} reason={c.get('reason')} status={c.get('status')} "
                f"reviewed={c.get('reviewed_candidates')} protected={c.get('protected_candidates')} "
                f"demoted={c.get('demoted_candidates')} proposed={c.get('proposals_created')} "
                f"prom_eval={c.get('promotion_evaluations')}"
            )

    graph = diag.get("graph") or {}
    if graph:
        print("\n[GRAPH]")
        print(f"nodes={graph.get('nodes')} edges={graph.get('edges')}")
    print("===============================")
...
def _print_hud(hud: dict) -> None:
    ...
    adaptive_bits = []
    for key in ("protected_candidates", "durable_episodic_candidates", "collation_runs", "collation_reviewed", "collation_protected", "collation_proposed"):
        if last.get(key, 0):
            adaptive_bits.append(f"{key}={last.get(key, 0)}")
    if adaptive_bits:
        print("      knowledge_evolution: " + " ".join(adaptive_bits))
